...
{
    [TestClass()]
    public class PrimeGeneratorVersion4_1Tests
    {
        [TestMethod()]
        public void GeneratePrimeNumbersTest()
        {
            //Assert.Fail();
            int[] nullArray = PrimeGeneratorVersion4_1.GeneratePrimeNumbers(0);
            Assert.AreEqual(0, nullArray.Length);

            int[] minArray = PrimeGeneratorVersion4_1.GeneratePrimeNumbers(2);
            Assert.AreEqual(1, minArray.Length);
            Assert.AreEqual(2, minArray[0]);

            int[] threeArray = PrimeGeneratorVersion4_1.GeneratePrimeNumbers(3);
            Assert.AreEqual(2, threeArray.Length);
            Assert.AreEqual(2, threeArray[0]);
            Assert.AreEqual(3, threeArray[1]);

            int[] centArray = PrimeGeneratorVersion4_1.GeneratePrimeNumbers(100);
